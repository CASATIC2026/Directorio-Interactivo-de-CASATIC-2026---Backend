...
import {
  ...
  PanelLeftClose, PanelLeft, Bell, Search, Briefcase
} from 'lucide-react';
...
export default function AdminLayout() {
  ...
  const menuItems = [
    ...(isAdmin ? [{ to: '/admin', label: 'Dashboard', icon: LayoutDashboard, exact: true }] : []),
    ...(isAdmin ? [{ to: '/admin/estadisticas', label: 'Estadísticas', icon: Search }] : []),
    ...(isAdmin ? [{ to: '/admin/socios', label: 'Socios', icon: Building2 }] : []),
    ...(isAdmin ? [{ to: '/admin/usuarios', label: 'Usuarios', icon: Users }] : []),
    ...(!isAdmin ? [{ to: '/admin/mi-empresa', label: 'Mi Empresa', icon: Briefcase }] : []),
  ];
